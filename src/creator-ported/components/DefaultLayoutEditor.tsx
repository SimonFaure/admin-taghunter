...
interface DefaultLayoutEditorProps {
  scenarioType: string;
  layoutType?: 'instruction' | 'game';
  scenarioId?: string;
  onBack: () => void;
}
...
function buildImagesList(media: any, numQuests = 0, scenarioType = ''): { id: string; name: string; filename: string }[] {
  const list: { id: string; name: string; filename: string }[] = [];

  if (media?.images?.game_instructions_image) {
    list.push({ id: 'game_instructions_image', name: 'Game Instructions Image', filename: media.images.game_instructions_image });
  }
  if (media?.images?.game_instructions_button_image) {
    list.push({ id: 'game_instructions_button_image', name: 'Instructions Button Image', filename: media.images.game_instructions_button_image });
  }
  if (media?.images?.game_refresh_button_image) {
    list.push({ id: 'game_refresh_button_image', name: 'Refresh Button Image', filename: media.images.game_refresh_button_image });
  }
  if (media?.images?.time_background_image) {
    list.push({ id: 'time_background_image', name: 'Time Background Image', filename: media.images.time_background_image });
  }
  if (media?.images?.score_background_image) {
    list.push({ id: 'score_background_image', name: 'Score Background Image', filename: media.images.score_background_image });
  }
  if (media?.images?.enigmas_header_image) {
    list.push({ id: 'enigmas_header_image', name: 'Enigmas Header Image', filename: media.images.enigmas_header_image });
  }
  if (media?.overscores?.[0]?.image_overscore_step) {
    list.push({ id: 'overscore_step_1_image', name: 'Overscore Step 1 Image', filename: media.overscores[0].image_overscore_step });
  }
  if (media?.enigmas?.[0]?.good_answer_image) {
    list.push({ id: 'enigma_1_good_answer_image', name: 'Enigma 1 Good Answer Image', filename: media.enigmas[0].good_answer_image });
  }

  const imgs = media?.game_media_images || media?.images || {};
  if (imgs.combo_image) list.push({ id: 'combo_image', name: 'Combos Container', filename: imgs.combo_image });

  const malusContainerFile = imgs.malus_container;
  const malusFile = imgs.malus_image;
  const anyContainerFile = imgs.combo_image || imgs.team_name_container_image || imgs.rectangle_image || imgs.score_image;
  const isTagquest = scenarioType === 'tagquest';

  if (malusContainerFile || malusFile || isTagquest) {
    const resolvedMalus = malusContainerFile || malusFile || anyContainerFile;
    if (resolvedMalus) {
      list.push({ id: 'malus_image', name: 'Malus Container', filename: resolvedMalus });
      list.push({ id: 'late_malus_container_image', name: 'Late Malus Container', filename: resolvedMalus });
    }
  }
  if (malusFile) {
    list.push({ id: 'malus_icon_image', name: 'Malus Image', filename: malusFile });
  }
  if (imgs.late_malus_image) {
    list.push({ id: 'late_malus_icon_image', name: 'Late Malus Image', filename: imgs.late_malus_image });
  }

  const rectImg = imgs.team_name_container_image || imgs.rectangle_image;
  if (rectImg) list.push({ id: 'team_name_container_image', name: 'Team Name Container', filename: rectImg });
  const timerImg = imgs.timer_container_image || rectImg;
  if (timerImg) list.push({ id: 'timer_container_image', name: 'Timer Container', filename: timerImg });
  if (imgs.quest_counter_image && numQuests > 0) {
    const questsMedia: any[] = media?.quests || [];
    const firstQuestMainImage = questsMedia[0]?.main_image || imgs.quest_counter_image;
    list.push({ id: 'animation_quest_image', name: 'Animation Quest Main Image', filename: firstQuestMainImage });
    for (let i = 1; i <= numQuests; i++) {
      list.push({ id: `placement_${i}`, name: `Placement ${i}`, filename: imgs.quest_counter_image });
      const questMainImage = questsMedia[i - 1]?.main_image || imgs.quest_counter_image;
      list.push({ id: `quest_${i}_image`, name: `Quest ${i} Main Image`, filename: questMainImage });
    }
  }
  if (imgs.score_image) list.push({ id: 'score_image', name: 'Score Image', filename: imgs.score_image });

  return list;
}

export function DefaultLayoutEditor({ scenarioType, layoutType, scenarioId, onBack }: DefaultLayoutEditorProps) {
  const [templateScenarioId, setTemplateScenarioId] = useState<string>('');
  ...
  const [layoutSource, setLayoutSource] = useState<'scenario' | 'default' | 'none'>('none');
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set());
  const [numQuests, setNumQuests] = useState<number>(0);
  ...
  const groups: GroupDef[] = isTagquest ? buildGroups(numQuests) : [];
  ...
  const visibleElements = elements.filter(element => {
    ...
    if (layoutType === 'instruction') return instructionLayoutImages.includes(element.id);
  ...
  const getMetaKey = () => {
    if (layoutType) return `${scenarioType}_layout_${layoutType}`;
    return `${scenarioType}_layout`;
  };
  ...
  const calculateImageBounds = () => {
    ...
    if (imgAspectRatio > containerAspectRatio) {
      ...
      actualWidth = containerHeight * imgAspectRatio;
      offsetY = 0;
  ...
  useEffect(() => {
    ...
    const delayed = () => setTimeout(updateContainerHeight, 50);
    delayed();
  ...
  const loadTemplateScenario = async () => {
    try {
      ...
      if (media?.images?.background_image) {
        setBackgroundImage(getMediaUrl(data.id, media.images.background_image));
      }

      setAvailableImages(buildImagesList(media, nQuests, scenarioType));
    } catch (error) {
      console.error('Error loading template scenario:', error);
    }
  };
  ...
  const loadScenarioData = async (id: string) => {
    try {
      ...
      if (!data) {
        setAlert({ type: 'error', message: 'Scenario not found' });
        return;
      }
      ...
      setAlert({ type: 'error', message: 'Failed to load scenario data' });
    }
  };
  ...
  const loadDefaultLayout = async () => {
    try {
      if (scenarioId) {
        ...
        if (data?.scenario_layout) {
          const layoutData = data.scenario_layout as any;
          if (layoutData.elements && layoutData.elements.length > 0) {
            setElements(layoutData.elements);
            setLayoutSource('scenario');
            return;
          }
        }
        ...
        if (defaultData?.value) {
          const layoutData = defaultData.value as any;
          if (layoutData.elements && layoutData.elements.length > 0) {
            setElements(layoutData.elements);
            setLayoutSource('default');
            if (defaultData.version) setCurrentVersion(Number(defaultData.version));
            if ((defaultData as any).layout_uniqid) setLayoutUniqid((defaultData as any).layout_uniqid);
          } else {
        ...
        if (data?.value) {
          const layoutData = data.value as any;
          if (layoutData.elements) setElements(layoutData.elements);
        }
        if (data?.version) setCurrentVersion(Number(data.version));
        if ((data as any)?.layout_uniqid) setLayoutUniqid((data as any).layout_uniqid);
      }
    } catch (error) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDragging) {
      ...
      setElements(prev => {
        const current = prev.find(el => el.id === selectedElement);
        if (!current) return prev;
        const newWidth = Math.max(2, Math.min(100 - current.x, current.width + deltaX));
        const newHeight = Math.max(1, Math.min(100 - current.y, current.height + deltaY));
    ...
    setElements(prev => {
      ...
      for (let n = 2; n <= questGroups.length; n++) {
        ...
        if (nItemDef.type === 'image') {
          ...
          if (!imgInfo) continue;
          const placement1 = updated.find(p => p.id === 'placement_1');
          ...
          const srcText = sourceEl as TextElement;
          const newEl: TextElement = {
            ...srcText,
            id: counterpartId,
            name: nItemDef.name,
  ...
  const alignQuestsVertically = () => {
    setElements(prev => alignQuestMainImagesVertically(prev, numQuests));
  };
  ...
  const syncQuestText = <K extends 'fontSize' | 'fontFamily' | 'color'>(
    prev: LayoutElement[],
    elementId: string,
    key: K,
    value: any
    ...
    const questGroups = groups.filter(g => g.questIndex !== undefined);
    const updated = prev.map(el => el.id === elementId ? { ...el, [key]: value } : el);
    if (!isQuest1Text || questGroups.length <= 1) return updated;
    ...
    return updated.map(el => {
      ...
      if (el.id === `quest_${elQIdx}_${role}`) return { ...el, [key]: value };
      return el;
    });
  };
  ...
  const updateFontSize = (elementId: string, delta: number) => {
    setElements(prev => {
      const current = prev.find(el => el.id === elementId);
      if (!current || current.type !== 'text') return prev;
      const newSize = Math.max(0.5, Math.min(200, parseFloat((current.fontSize + delta).toFixed(1))));
      return syncQuestText(prev, elementId, 'fontSize', newSize);
    });
  };
  ...
  const updateColor = (elementId: string, color: string) => {
    setElements(prev => syncQuestText(prev, elementId, 'color', color));
  };

  const updateFontFamily = (elementId: string, fontFamily: string) => {
    setElements(prev => syncQuestText(prev, elementId, 'fontFamily', fontFamily));
  };
  ...
  const saveLayout = async () => {
    try {
      ...
      const currentVersionFromDb = existingConfig?.version || 1.0;
      const newVersion = Number((currentVersionFromDb + 0.1).toFixed(1));
  ...
  const getTitle = () => {
    if (scenarioId && scenarioTitle) return `Layout Editor - ${scenarioTitle}`;
    const typeLabel = scenarioType.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    if (layoutType) return `${typeLabel} - ${layoutType === 'instruction' ? 'Instruction' : 'Game'} Layout`;
    return `${typeLabel} - Layout`;
  };
    ...
    <div className={`min-h-screen bg-gray-900 text-white ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
      <div className="border-b border-gray-800 bg-gray-950">
        <div className="px-3 py-2 flex items-center gap-2">
          <button onClick={onBack} className="flex items-center gap-1.5 text-gray-400 hover:text-white transition-colors text-sm px-2 py-1.5 rounded hover:bg-gray-800 flex-shrink-0">
            <ArrowLeft className="w-4 h-4" />
            Back
          </button>
          <div className="w-px h-5 bg-gray-700 flex-shrink-0" />
          <div className="flex items-center gap-2 min-w-0 flex-shrink-0">
            <h1 className="text-sm font-semibold text-white truncate max-w-[180px]">{getTitle()}</h1>
            {!scenarioId && <span className="text-xs text-gray-500 flex-shrink-0">v{currentVersion.toFixed(1)}</span>}
            {scenarioId && layoutSource === 'scenario' && (
              <span className="text-xs px-1.5 py-0.5 bg-green-900/50 text-green-400 rounded border border-green-800 flex-shrink-0">Scenario</span>
            )}
            {scenarioId && layoutSource === 'default' && (
              <span className="text-xs px-1.5 py-0.5 bg-blue-900/50 text-blue-400 rounded border border-blue-800 flex-shrink-0">Default</span>
            )}
            {scenarioId && layoutSource === 'none' && (
              <span className="text-xs px-1.5 py-0.5 bg-yellow-900/50 text-yellow-400 rounded border border-yellow-800 flex-shrink-0">New</span>
            )}
          </div>
      ...
      <div className="flex h-[calc(100vh-45px)] relative">
        {/* Sidebar */}
        <div className={`bg-gray-950 border-r border-gray-800 overflow-y-auto transition-all duration-300 flex-shrink-0 ${sidebarCollapsed ? 'w-0' : 'w-80'}`}>
          <div className={`p-4 ${sidebarCollapsed ? 'hidden' : ''}`}>
            {layoutType && (
              <p className="text-xs text-gray-500 mb-4">({layoutType === 'instruction' ? 'Instruction' : 'Game'} Layout)</p>
            )}
                    ...
                    <div key={group.id} className="rounded-lg border border-gray-800 overflow-hidden">
                      ...
                      {isExpanded && (
                        <div className="bg-gray-900 border-t border-gray-800">
                          {group.questIndex === 1 && numQuests > 1 && (
                            <div className="px-3 pt-2 pb-1">
                              <p className="text-xs text-amber-500/80 italic">Changes propagate to all {numQuests} quests</p>
                            </div>
                          )}
                          {mainImgInfo && (
                            <div className="px-3 pt-2 pb-1">
                              <div className="w-full h-16 bg-gray-800 rounded overflow-hidden">
                                <img
                                  src={getMediaUrl(templateScenarioId, mainImgInfo.filename)}
                          ...
                          {((!allPlaced) || placedCount > 0 || (group.questIndex === 1 && numQuests > 1)) && (
                            <div className="px-2 pt-2 pb-1 flex items-center gap-1.5 flex-wrap">
                              ...
                              {!allPlaced && (
                                ...
                                >
                                  {group.questIndex === 1 && numQuests > 1 ? 'Add All Quests' : 'Add All'}
                                </button>
                              )}
                              {placedCount > 0 && (
                                <button
                                  ...
                                  title={group.questIndex === 1 && numQuests > 1 ? 'Remove all quest elements' : 'Remove all elements in this group'}
                                >
                                  {group.questIndex === 1 && numQuests > 1 ? 'Remove All Quests' : 'Remove All'}
                                </button>
                              )}
                            </div>
                          )}
                          <div className="px-2 py-2 space-y-1">
                            {groupItems.map(item => {
                              ...
                              const element = visibleElements.find(el => el.id === item.id);
                              const isSelected = selectedElement === item.id;
                              const globalIndex = element ? elements.findIndex(el => el.id === item.id) : -1;
                              const isDragOver = dragOverIndex === globalIndex && dragReorderIndex !== globalIndex;
                              return (
                                <div
                                  key={item.id}
                                  draggable={!!element}
                                  onDragStart={element ? (e) => { setDragReorderIndex(globalIndex); e.dataTransfer.effectAllowed = 'move'; } : undefined}
                                  onDragOver={element ? (e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDragOverIndex(globalIndex); } : undefined}
                                  onDragLeave={element ? () => setDragOverIndex(null) : undefined}
                                  onDrop={element ? (e) => { e.preventDefault(); if (dragReorderIndex !== null) reorderElements(dragReorderIndex, globalIndex); setDragReorderIndex(null); setDragOverIndex(null); } : undefined}
                                  onDragEnd={element ? () => { setDragReorderIndex(null); setDragOverIndex(null); } : undefined}
                                  className={`rounded-md transition-all ${
                                    dragReorderIndex === globalIndex ? 'opacity-40 scale-95' :
                                    isDragOver ? 'ring-1 ring-emerald-400' :
                                    isSelected ? 'bg-blue-900/40 ring-1 ring-blue-600' : 'bg-gray-800/60'
                                  }`}
                                >
                                  <div
                                  ...
                                  >
                                    {element && <GripVertical size={10} className="flex-shrink-0 text-gray-600" />}
                                    {item.type === 'text'
                                    ...
                                    {placed && element ? (
                                      <div className="flex items-center gap-1.5 flex-shrink-0">
                                        {element && <span className="text-xs text-gray-600">z:{globalIndex + 1}</span>}
                                        <button
                                      ...
                                      {element.type === 'text' && (
                                        <>
                                          <div className="flex items-center gap-2 pl-1">
                                            <span className="text-xs text-gray-400 w-10 flex-shrink-0">Font:</span>
                                            <select
                                              value={element.fontFamily}
                                              onChange={(e) => { e.stopPropagation(); updateFontFamily(element.id, e.target.value); }}
                                          ...
                                          <div className="flex items-center gap-2 pl-1">
                                            <span className="text-xs text-gray-400 w-10 flex-shrink-0">Size:</span>
                                            <button onClick={(e) => { e.stopPropagation(); updateFontSize(element.id, -0.5); }} className="w-6 h-6 bg-gray-700 hover:bg-gray-600 rounded text-xs flex items-center justify-center flex-shrink-0">−</button>
                                            <input
                                              type="number"
                                              min="0.5"
                                              max="200"
                                              ...
                                              onChange={(e) => {
                                                ...
                                                if (!isNaN(val)) {
                                                  setElements(prev => {
                                                    const newSize = Math.max(0.5, Math.min(200, parseFloat(val.toFixed(1))));
                                                    return syncQuestText(prev, element.id, 'fontSize', newSize);
                                                  });
                                                }
                                              }}
                                              className="w-14 text-xs text-center bg-gray-700 border border-gray-600 rounded px-1 py-0.5 text-white focus:outline-none focus:border-emerald-500 flex-shrink-0"
                                            />
                                            <span className="text-xs text-gray-500 flex-shrink-0">%</span>
                                            <button onClick={(e) => { e.stopPropagation(); updateFontSize(element.id, 0.5); }} className="w-6 h-6 bg-gray-700 hover:bg-gray-600 rounded text-xs flex items-center justify-center flex-shrink-0">+</button>
                                          </div>
                                          <div className="flex items-center gap-2 pl-1">
                                            <span className="text-xs text-gray-400 w-10 flex-shrink-0">Color:</span>
                                            <div className="flex items-center gap-1.5 flex-1">
                                              <input
                                                type="color"
                                                value={element.color}
                                                onChange={(e) => { e.stopPropagation(); updateColor(element.id, e.target.value); }}
                                              ...
                                              <input
                                                ...
                                                value={element.color}
                                                onChange={(e) => { e.stopPropagation(); updateColor(element.id, e.target.value); }}
                                                className="flex-1 bg-gray-700 border border-gray-600 text-white text-xs rounded px-1.5 py-1 font-mono"
                                                placeholder="#ffffff"
                                              />
                                            </div>
                                          </div>
        ...
        >
          {zoom !== 1 && (
            <div className="absolute top-3 right-3 z-30 bg-gray-900/80 text-white text-xs px-2 py-1 rounded pointer-events-none">
              {Math.round(zoom * 100)}%
            </div>
          )}
          <div
          ...
          >
            ...
            {visibleElements.map((element, idx) => {
              ...
              const elHeightPx = containerHeight * (elHeight / 100);
              return (
              <div
                key={element.id}
              ...
              >
                {element.type === 'image' ? (
                  <img
                    src={getMediaUrl(templateScenarioId, element.filename)}
                    alt={element.name}
                  ...
                  <div
                    ...
                    style={{
                      background: selectedElement === element.id ? 'rgba(16, 185, 129, 0.08)' : 'transparent',
                ...
                {selectedElement === element.id && (
                  <>
                    <div className={`absolute -top-6 left-0 text-white text-xs px-2 py-1 rounded whitespace-nowrap ${element.type === 'text' ? 'bg-emerald-600' : 'bg-blue-600'}`}>
                      {element.name}
                    </div>
                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full border border-white shadow-sm" style={{ background: element.type === 'text' ? '#10b981' : '#3b82f6' }} />
                    <div className="absolute top-1/2 -translate-y-1/2 -right-1 w-2 h-2 rounded-full border border-white shadow-sm" style={{ background: element.type === 'text' ? '#10b981' : '#3b82f6' }} />
                    <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full border border-white shadow-sm" style={{ background: element.type === 'text' ? '#10b981' : '#3b82f6' }} />
                    <div className="absolute top-1/2 -translate-y-1/2 -left-1 w-2 h-2 rounded-full border border-white shadow-sm" style={{ background: element.type === 'text' ? '#10b981' : '#3b82f6' }} />
                    <div
                      className={`absolute bottom-0 right-0 w-4 h-4 cursor-nwse-resize flex items-center justify-center ${element.type === 'text' ? 'bg-emerald-500' : 'bg-blue-500'}`}
