...
export function AdminConfigPage() {
  ...
  const loadClientsFromDatabase = async () => {
    ...
    if (!email) {
      console.warn('Skipping client fetch: user not logged in');
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !body.success) {
        console.error('Error loading clients from database:', body.error || response.statusText);
        return;
      }

      const mapped: Client[] = (body.data as AdminClient[])
        .map((c) => ({
          id: String(c.id),
      ...
      if (mapped.length > 0) {
        setClients([DEFAULT_CLIENT, ...mapped]);
      }
    } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Admin Status: <span className={isAdmin ? 'text-green-400' : 'text-red-400'}>
                  {isAdmin ? 'Active' : 'Not logged in as admin'}
                </span>
              </p>
            </div>
          </div>
